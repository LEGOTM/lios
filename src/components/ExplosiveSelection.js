...
const ExplosiveSelection = ({ setStep }) => {
  ...
  useEffect(() => {
    if (selectedExplosive && step === 1) {
      tg.MainButton.text = "Далее";
      tg.MainButton.show();
      tg.MainButton.offClick(handleNextStep);
      tg.MainButton.onClick(handleNextStep);
    }
  }, [selectedExplosive, step]);

  useEffect(() => {
    if (selectedMaterial && step === 2) {
      tg.MainButton.text = "Далее";
      tg.MainButton.show();
      tg.MainButton.offClick(handleNextStep);
      tg.MainButton.onClick(handleNextStep);
  ...
  const formatResultText = () => {
    ...
    if (amount === "Невозможно") {
      return (
        <strong>
          Невозможно разрушить выбранный объект данной взрывчаткой.
        </strong>
      );
    }
